...
DATE: Thursday, January 08, 2015     TIME: 9:57:50 AM
PROJECT: HigelmcgovernC_SAS_Project_010715
PROJECT PATH: P:\QAC\qac200\students\chigelmcgove\HigelmcgovernC_SAS_Project_010715.egp
---------------------------------------- */
...
%LET _CLIENTPROJECTPATH='P:\QAC\qac200\students\chigelmcgove\HigelmcgovernC_SAS_Project_010715.egp';
%LET _CLIENTPROJECTNAME='HigelmcgovernC_SAS_Project_010715.egp';
...
GOPTIONS ACCESSIBLE;
%_eg_conditional_dropds(CAHMDATA.'Refined Variables'n);

PROC SQL;
   CREATE TABLE CAHMDATA.'Refined Variables'n(label="Refined Variables") AS 
   SELECT t1.ADAPPT42, 
          t1.ADCAPE42, 
          t1.ADCLIM42, 
          t1.ADCMPD42, 
...
/* -------------------------------------------------------------------
   ...
   Generated on: Thursday, January 08, 2015 at 9:52:05 AM
...
PROC DATASETS NOLIST NODETAILS; 
   CONTENTS DATA=CAHMDATA.'REFINED VARIABLES'n OUT=WORK.SUCOUT1;

RUN;

DATA WORK.'CONTContentsForREFINED VARIABLES'n(LABEL="Contents Details for REFINED VARIABLES");
   SET WORK.SUCOUT1;
RUN;
...
CREATE VIEW WORK.SCVIEW AS 
	SELECT DISTINCT memname LABEL="Table Name", 
			...
			typemem LABEL="Data Set Type" FROM WORK.'CONTContentsForREFINED VARIABLES'n
	ORDER BY memname ; 

CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.SCVIEW
		WHERE memname='REFINED VARIABLES';
QUIT;
...
PROC SORT DATA=WORK.'CONTContentsForREFINED VARIABLES'n OUT=WORK.'CONTContentsForREFINED VARIABLES'n;
   BY memname name;
RUN;
...
CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.'CONTContentsForREFINED VARIABLES'n
		WHERE memname='REFINED VARIABLES';
QUIT;
...
/* -------------------------------------------------------------------
   ...
   Generated on: Thursday, January 08, 2015 at 9:52:10 AM
